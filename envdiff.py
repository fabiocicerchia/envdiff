...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"envdiff: error: source command exited {exc.returncode}", file=sys.stderr)  # noqa: T201
        return EXIT_UNAVAILABLE
    except OSError as exc:
        # The tool's error output
        print(f"envdiff: error: {exc.filename}: {exc.strerror}", file=sys.stderr)  # noqa: T201
        return EXIT_NOINPUT
